...
export function RoleOverview() {
  ...
  const addRole = () => {
    ...
    setRoles([...roles, { name: newRoleName, description: '', permissions: [], actions: [], userCount: 0, color }])
              ...
              <div key={role.name} className={!isLast ? 'border-b border-fd-border' : ''}>
                ...
                >
                  ...
                  <div className="flex flex-1 items-center gap-3 px-4 py-3.5">
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                      className={`shrink-0 text-fd-muted-foreground transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`}>
                ...
                {isExpanded && (
                  <div className="border-t border-fd-border bg-fd-background">
                    <div className="flex">
                      {/* Color accent continuation */}
                      <div className={`w-1 shrink-0 ${role.color}`} />
