...
One library to connect your frontend to any database -- with authentication, row-level permissions, and type safety built in. Deploy to Node.js or edge runtimes.
...
|---|---|
| `@superapp/backend` | Connects to Postgres, MySQL, SQLite, or CSV. Handles auth, enforces row-level permissions, runs queries. Supports two execution modes: **DuckDB** (cross-database joins) or **direct** (edge-compatible, no native dependencies). |
...
```text
  ...
  │         ▼                                                 │
  │  3. Execute ────── run permission-filtered SQL             │
  │         ├── DuckDB mode (default): via DuckDB ATTACH      │
  │         └── Direct mode: via native database driver        │
  └───────────────────────────┬───────────────────────────────┘
                              │
                ┌─────────────┼─────────────┐
                ▼             ▼             ▼
...
**Edge deployment** -- add one line to deploy to Cloudflare Workers, Vercel Edge, or Deno:

```typescript
const engine = createEngine({
  executor: 'direct',  // ← skip DuckDB, use native pg driver
  integrations: [postgresProvider],
  connections: {
    main: { type: 'postgres', url: process.env.PG_URL! },
  },
  // ... same auth, permissions, roles
})
```
...
- [Direct Connections](/docs/advanced/direct-connections) -- Deploy to edge runtimes without DuckDB.
