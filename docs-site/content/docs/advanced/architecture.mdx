...
```text
  ...
  │         │                         │
  │         ▼                         │
  │  Query Execution (native drivers)  │
  └─────────┬─────────────────────────┘
            │
    ┌───────┼───────┬───────┐
    ▼       ▼       ▼       ▼
...
- **Executes the permission-filtered SQL** directly against the target database using its native driver.
...
|---|---|---|
| Query execution | Native drivers | Direct connections via `pg`, `mysql2`, `better-sqlite3` |
...
```text
  createEngine()
  ├── integrations[]  → Database providers     ──→ Native driver pools
  ├── connections{}   → Named DB configs       ──→ Native driver pools
  ├── auth            → AuthProvider
  │                     ├── verifyToken()
  │                     ├── findUser()
  │                     └── resolveSession()
  ...
  ├── connections{}   → Connection pool config
...
4. **Query Execution** -- Executes the permission-filtered SQL directly against the target database (Postgres, MySQL, SQLite, or CSV) using its native driver.
