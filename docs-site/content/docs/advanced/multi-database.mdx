...
<Callout type="warn">
Cross-database joins require DuckDB mode (`executor: 'duckdb'`, the default). In [direct mode](/docs/advanced/direct-connections), each connection uses its own native driver and cannot join across databases.
</Callout>
...
```text
  Query: db.select().from(schema.orders)
    ...
    ▼
  DuckDB Engine
    ├── ATTACH AS main      → Postgres
    ├── ATTACH AS warehouse → MySQL
    ├── ATTACH AS legacy    → SQLite
    └── ATTACH AS imports   → CSV
```
...
This namespacing is consistent across the entire system and works in both DuckDB and direct mode:
...
Cross-database includes only work in DuckDB mode:

```typescript
// DuckDB mode only — cross-database join
...
JWT verification
  │
  ▼
Permission evaluation (validate SQL, inject filters, restrict columns, check, preset)
  │
  ▼
Query execution → routes to correct database
  │
  ▼
Response
```
...
Both permissions compile to the same CASL ability structure and produce the same kind of SQL WHERE clause. The executor handles routing transparently.

## Provider Types

Providers are integration modules that teach the engine how to connect to a specific database type.

### Built-in Providers

| Provider | Import | Connection Config | DuckDB Extension | Direct Driver |
|---|---|---|---|---|
| `postgresProvider` | `@superapp/backend/integrations/postgres` | `{ type: 'postgres', url: string }` | `postgres_scanner` | `pg` |
| `mysqlProvider` | `@superapp/backend/integrations/mysql` | `{ type: 'mysql', url: string }` | `mysql_scanner` | `mysql2` |
| `sqliteProvider` | `@superapp/backend/integrations/sqlite` | `{ type: 'sqlite', path: string }` | `sqlite_scanner` | `better-sqlite3` |
| `csvProvider` | `@superapp/backend/integrations/csv` | `{ type: 'csv', directory: string }` | built-in | N/A (DuckDB only) |

### Native vs Custom Providers

**Native providers** use DuckDB's built-in scanner extensions in DuckDB mode, or standard database drivers in direct mode. In DuckDB mode, they support push-down predicates, which means DuckDB can push WHERE clauses down to the source database for efficient filtering.
...
```typescript
// DuckDB mode: DuckDB pushes the WHERE clause to Postgres
...
// Direct mode: same query, same result — SQL sent directly to Postgres via pg driver
// Performance is equivalent for single-database queries

// CSV provider: all rows loaded, then filtered in DuckDB (DuckDB mode only)
...
Providers are registered in the `integrations` array. Only register the providers you need:
...
Because all databases are attached to the same DuckDB instance, you can join across them in DuckDB mode:

```typescript
// Join Postgres orders with MySQL analytics events (DuckDB mode only)
...
<Callout>
Cross-database joins are not available in direct mode. If you need this feature, use `executor: 'duckdb'` (the default).
</Callout>

## Performance Considerations

- **Native providers** (Postgres, MySQL, SQLite) benefit from predicate push-down in DuckDB mode. In direct mode, queries go straight to the database driver with no intermediary.
- **CSV sources** are loaded fully into DuckDB memory. Use the `duckdb.maxMemory` setting to control how much memory DuckDB can use. CSV is not available in direct mode.
...
- **Connection pooling** depends on the execution mode. In DuckDB mode, DuckDB maintains its own connection pool. In direct mode, each database driver manages its own pool via the `pool` config on each connection.
