...
```text
  ...
  6a. Middleware (before next) — Custom TypeScript logic (optional)
    │
    ▼
  7. Query Execution — next() runs the permission-filtered SQL
    │   ├── DuckDB mode: routes through DuckDB to attached databases
    │   └── Direct mode: sends SQL to the native database driver
    │
    ▼
...
Steps 1--6 and 8--9 are identical regardless of execution mode. Only step 7 differs based on the `executor` setting.
...
The permission-filtered SQL is executed by the configured executor.

**DuckDB mode** (`executor: 'duckdb'`, default): DuckDB routes the query to the appropriate attached database (Postgres, MySQL, SQLite, or CSV):
...
**Direct mode** (`executor: 'direct'`): The native database driver executes the query against the target database:
...
connections: {
  main: {
    ...
    pool: { max: 10 },  // native driver pool settings
  },
}
```
...
See [Actions](/docs/backend/actions) for details.
...
| `408` | Query Execution | Query timeout exceeded |
