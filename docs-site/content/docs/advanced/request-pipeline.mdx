...
<Callout type="info">
This pipeline applies to both HTTP (`POST /data`) and [PostgreSQL wire protocol](/docs/backend/adapters/pg-wire) requests. The wire protocol replaces steps 1-3 with TCP connection handshake + PG protocol parsing, but steps 4-9 are identical.
</Callout>
...
The permission-filtered SQL is executed directly against the target database using its native driver (Postgres, MySQL, etc.):
