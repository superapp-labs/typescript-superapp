...
```text
  ...
  6. Permission Check — Validate SQL → Inject filters → Restrict columns → Check → Preset
    │
    ▼
  6a. beforeExecute Hook — Custom TypeScript logic (optional)
    │
    ▼
  7. DuckDB Execution — Run permission-filtered SQL against attached databases
    │
    ▼
  7a. afterExecute Hook — Custom TypeScript logic (optional)
    │
    ▼
...
### 6a. beforeExecute Hook

If the matching permission defines a `hooks.beforeExecute` function, it runs now with the full query context (`user`, `db`, `table`, `operation`, `columns`, `input`, `filter`). The hook can:

- **Throw** to reject the request with `403 Forbidden`
- **Return** modified `filter`, `input`, or `columns` to alter the query
- **Return nothing** to pass through unchanged

See [Custom Hooks](/docs/backend/permissions/custom-hooks) for details.
...
### 7a. afterExecute Hook

If the matching permission defines a `hooks.afterExecute` function, it runs with the query results. The hook can:

- **Return an array** to replace the result set (transform, redact, or enrich rows)
- **Return nothing** to pass results through unchanged
- **Run side effects** like writing audit entries or triggering notifications
