...
```text
  ...
  6. Permission Check — Validate SQL → Inject filters → Restrict columns → Check → Preset
    │
    ▼
  6a. Guard — Custom TypeScript logic (optional)
    │
    ▼
  7. DuckDB Execution — Run permission-filtered SQL against attached databases
    │
    ▼
  7a. Transform — Custom TypeScript logic (optional)
    │
    ▼
...
If the matching permission defines a `guard` function, it runs now with the full query context (`user`, `db`, `table`, `operation`, `columns`, `query`, `input`, `filter`). The guard can:
...
See [Guard & Transform](/docs/backend/permissions/guard-and-transform) for details.
...
If the matching permission defines a `transform` function, it runs with the query results. The transform can:
