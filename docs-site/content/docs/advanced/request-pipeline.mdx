...
If the matching permission defines a `middleware` function, it runs now with the full query context (`user`, `db`, `table`, `operation`, `columns`, `query`, `input`, `filter`) and a `next()` function. Before calling `next()`, the middleware can:

- **Throw** to reject the request with `403 Forbidden`
- **Mutate `ctx`** to modify `filter`, `input`, or `columns` before execution
- **Run queries** via `ctx.db` to look up related data
