...
<Callout type="info">
DuckDB is optional. If you don't need cross-database joins or CSV querying, you can use [direct connections](/docs/advanced/direct-connections) instead -- set `executor: 'direct'` to bypass DuckDB entirely and deploy to edge runtimes.
</Callout>

When `executor` is `'duckdb'` (the default), the engine uses DuckDB as its query execution layer. All connected databases (Postgres, MySQL, SQLite, CSV) are attached to DuckDB, which handles cross-database joins and query execution.
...
<Callout>
The `duckdb` config block is ignored when `executor: 'direct'` is set. In direct mode, connection pooling is configured per-connection. See [Direct Connections](/docs/advanced/direct-connections).
</Callout>
