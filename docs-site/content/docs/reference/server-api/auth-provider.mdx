...
where: { customer_id: { $eq: '$user.customer_id' } }             // 'cust_002'
where: { organization_id: { $in: '$user.org_ids' } }             // ['org_1', 'org_2']
where: { team_id: { $in: '$user.team_ids' } }                    // ['team_a', 'team_b']
overwrite: { created_by: '$user.id' }                             // 'usr_42'
overwrite: { organization_id: '$user.current_org_id' }            // 'org_1'
```
