...
interface Permission {
  ...
  /** Programmatic gatekeeper — runs before query execution */
  guard?: (ctx: QueryContext) => Promise<GuardResult | void>

  /** Programmatic result shaping — runs after query execution */
...
| | |
|---|---|
| **Type** | `(ctx: QueryContext) => Promise<GuardResult \| void>` |
| **Required** | No |
| **Applies to** | All operations |

A function that runs after permission checks but before query execution. Receives the full query context (`user`, `db`, `table`, `operation`, `columns`, `query`, `input`, `filter`). Can deny the request by throwing, or modify the query by returning `{ filter, input, columns }`.
...
| | |
|---|---|
| **Type** | `(ctx: TransformContext) => Promise<any[] \| void>` |
| **Required** | No |
| **Applies to** | All operations |

A function that runs after query execution. Receives the query context plus `rows`. Return an array to replace the result set, or void to pass through.
...
See [Guard & Transform](/docs/backend/permissions/guard-and-transform) for full documentation and examples.
