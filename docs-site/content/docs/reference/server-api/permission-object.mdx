...
interface Permission {
  ...
  middleware?: (ctx: QueryContext, next: () => Promise<any[]>) => Promise<any[]>
}
```
...
| | |
|---|---|
| **Type** | `(ctx: QueryContext, next: () => Promise<any[]>) => Promise<any[]>` |
| **Required** | No |
| **Applies to** | All operations |

A function that wraps query execution. Receives the full query context (`user`, `db`, `table`, `operation`, `columns`, `query`, `input`, `filter`) and a `next()` function. Call `next()` to execute the query and get rows back. Mutate `ctx` before `next()` to modify the query. Transform rows after `next()` to shape results.
