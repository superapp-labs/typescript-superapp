...
interface Permission {
  ...
  /** Named server-side functions callable by the client */
  actions?: Record<string, (
    context: { user: UserSession, db: DrizzleInstance },
    input: any,
  ) => Promise<any>>
}
```
...
See [Middleware](/docs/backend/permissions/middleware) for full documentation and examples.

### `actions`

| | |
|---|---|
| **Type** | `Record<string, (context, input) => Promise<any>>` |
| **Required** | No |
| **Applies to** | Custom operations called by name |

Named server-side functions that the client calls via `POST /actions/{actionName}`. Each action receives `{ user, db }` and the client's input. Actions inherit role-based access control from the permission but do not auto-apply `filter`, `check`, or `preset` — you scope queries yourself.
...
See [Actions](/docs/backend/permissions/actions) for full documentation and examples.
