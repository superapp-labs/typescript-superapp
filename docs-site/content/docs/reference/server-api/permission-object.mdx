...
Maximum rows this permission can return per query. The client's `limit` is capped to whichever is lower: the permission limit or the global `limits.maxLimit`.
