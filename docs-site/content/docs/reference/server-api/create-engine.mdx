...
| `actions` | `Record<string, Action>` | `{}` | Named server-side functions callable by the client via `POST /actions/{name}`. See [Actions](/docs/backend/actions). |
...
const engine = createEngine({
  ...
  roles: {
    ...
    warehouse_manager: ['view_own_orders'],
  },
  actions: {
    incrementStock: {
