...
| `executor` | `'duckdb' \| 'direct'` | `'duckdb'` | Query execution mode. `'duckdb'` routes all queries through DuckDB (supports cross-DB joins, CSV). `'direct'` uses native database drivers (edge-compatible, no DuckDB dependency). See [Direct Connections](/docs/advanced/direct-connections). |
...
| `actions` | `Record<string, Action>` | `{}` | Typed server-side functions callable by the client via `POST /actions/{name}`. Each action defines Zod `input`/`output` schemas and a `run` function. Access controlled through `roles` using the `action_` prefix. See [Actions](/docs/backend/actions). |
...
| `duckdb` | `DuckDBConfig` | See below | DuckDB engine configuration. Ignored when `executor: 'direct'`. |
...
Controls how queries are executed after permission filtering.

```typescript
// DuckDB mode (default) — queries routed through DuckDB
const engine = createEngine({
  connections: {
    main: { type: 'postgres', url: process.env.PG_URL! },
  },
...
// Direct mode — queries sent to native database drivers
const engine = createEngine({
  executor: 'direct',
  connections: {
    main: { type: 'postgres', url: process.env.PG_URL! },
...
| Value | Description |
|---|---|
| `'duckdb'` | Default. Routes queries through DuckDB. Supports cross-database joins, CSV file querying, and in-process OLAP. Requires a runtime that can load native binaries. |
| `'direct'` | Sends queries directly to native database drivers (`pg`, `mysql2`, `better-sqlite3`). No DuckDB dependency. Edge-compatible (Cloudflare Workers, Vercel Edge, Deno Deploy). Does not support cross-database joins or CSV connections. |
...
| Type | Config Shape | Direct Mode |
|---|---|---|
| `postgres` | `{ type: 'postgres', url: string, pool?: PoolConfig }` | Yes |
| `mysql` | `{ type: 'mysql', url: string, pool?: PoolConfig }` | Yes |
| `sqlite` | `{ type: 'sqlite', path: string }` | Yes |
| `csv` | `{ type: 'csv', directory: string }` | No (DuckDB only) |

In direct mode, connections support an optional `pool` config for native driver pooling:
...
Configure the DuckDB in-process engine. These settings are ignored when `executor: 'direct'`.
