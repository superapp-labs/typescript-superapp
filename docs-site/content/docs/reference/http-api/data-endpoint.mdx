...
| `where` | `object` | Conditional | `findMany`, `findOne`, `update`, `delete`, `count`, `aggregate` | Filter conditions using [filter operators](/docs/reference/client-api/filter-operators). Required for `findOne`, `update`, and `delete`. |
