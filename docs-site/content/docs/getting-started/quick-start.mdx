...
<Callout>
Not using Hono? superapp ships adapters for [Next.js](/docs/server/adapters), [Express](/docs/server/adapters), and a [generic handler](/docs/server/adapters) for Cloudflare Workers, Deno, and Bun.
</Callout>
...
- [Backend Configuration](/docs/server) — Full reference for `createEngine` options, connections, DuckDB tuning, and security settings.
- [Client Queries](/docs/client/queries) — All query methods: `db.select()`, `db.insert()`, `db.update()`, `db.delete()`.
- [Permissions](/docs/server/permissions) — Row-level filters, column restrictions, write checks, and preset values.
