...
sequenceDiagram
    ...
    Browser->>Backend: POST /auth/sign-in
    Backend-->>Browser: JWT token

    Browser->>Backend: POST /data (JWT + Drizzle query)
    Backend->>Backend: Verify JWT
    Backend->>Backend: Resolve session ($user)
    Backend->>Backend: Check permissions
    Backend->>Backend: Inject WHERE filters
    Backend->>Database: Execute SQL via DuckDB
    Database-->>Backend: Results
    Backend-->>Browser: Typed response
```
