...
`createAuth` from `@superapp/auth` initializes the auth client that handles sign-in, sign-up, session management, and token refresh. This package uses [better-auth](https://www.better-auth.com) by default, but supports custom adapters for any auth provider.
