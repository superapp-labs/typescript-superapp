...
3. **The server validates** the token, applies row-level permissions to the SQL, and executes directly against the database.
...
The `db` instance also exposes a typed `action()` method for calling server-side [actions](/docs/backend/actions):
...
This sends `POST /actions/incrementStock` with the input as JSON. The engine validates the JWT, checks the user's role for `action_incrementStock`, validates the input against the Zod schema, and returns the typed result. See [Actions](/docs/backend/actions) for details.
