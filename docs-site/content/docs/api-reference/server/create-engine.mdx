...
|---|---|---|---|
| `mode` | `'programmatic' \| 'managed'` | `'programmatic'` | Engine operation mode. `programmatic` = config in code, `managed` = config in admin UI. |
| `database` | `string` | `'./superapp.db'` | Path to the metadata database. Local file path for SQLite or a Turso URL. Stores sessions, roles, audit logs, and admin configuration. |
...
| `masterKey` | `string` | `undefined` | Master API key for admin access. Required for managed mode and admin endpoints. Set via environment variable, never hardcode. |
