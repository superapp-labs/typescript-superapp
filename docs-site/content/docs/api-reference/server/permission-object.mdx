...
A condition injected into the query's WHERE clause. The user cannot see, modify, or remove this filter. Supports all [filter operators](/docs/api-reference/client/filter-operators) and `$user.*` variable substitution.
