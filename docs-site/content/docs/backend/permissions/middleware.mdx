...
insert: {
  ...
  middleware: async ({ user, db, table, operation, columns, query, input, where }, next) => {
    ...
    const rows = await next({ where: { ...where, status: { $ne: 'archived' } } })
...
```typescript
middleware: async ({ where, columns, input }, next) => {
  return next({
...
view_patients: {
  table: 'main.patients',
  roles: ['staff', 'clinical'],
  select: {
    columns: '*',
    where: { facility_id: { $eq: '$user.facility_id' } },
...
manage_purchase_orders: {
  ...
  update: {
    ...
    middleware: async ({ user, db, operation, input, where }, next) => {
...
Middleware works alongside where clauses, validate rules, defaults, overwrite, and raw SQL. The engine applies them in order:
...
6. middleware   → Your TypeScript wraps the execution
