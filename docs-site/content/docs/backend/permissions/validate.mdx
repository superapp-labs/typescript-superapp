---
title: Validate
description: Validate data on insert and update operations before it reaches the database.
icon: CircleCheck
---
...
2. Apply each validate operator to the corresponding value
3. If any rule fails, return `403` with the failing field
...
## Validate with Session Variables
...
All [comparison operators](/docs/backend/permissions/operators) are available in validate rules: `$eq`, `$ne`, `$gt`, `$gte`, `$lt`, `$lte`, `$in`, `$nin`.
