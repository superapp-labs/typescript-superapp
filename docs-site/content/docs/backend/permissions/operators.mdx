...
title: Operators
icon: Code
description: Complete reference of MongoDB-style operators available in where clauses, validate rules, and defaults.
---

Permissions use MongoDB-style operators to define conditions. These operators work in both `where` (WHERE clauses) and `validate` (write validation) contexts.
...
Where clauses add SQL WHERE conditions. Multiple conditions at the same level are combined with AND:
...
Validate rules check request body values. The same operators apply, but they validate data rather than generating SQL:
