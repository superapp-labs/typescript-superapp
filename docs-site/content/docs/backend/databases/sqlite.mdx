...
Connect a local SQLite file with full CRUD support. Works in both DuckDB mode (via `sqlite_scanner`) and direct mode (via `better-sqlite3`). Ideal for development, testing, and embedded use cases.
...
When `executor` is `'duckdb'` (the default), the SQLite provider generates a DuckDB `ATTACH` statement on startup:
...
When `executor` is `'direct'`, the provider connects using `better-sqlite3`:
...
Direct mode with SQLite works in Node.js and Bun runtimes. For edge runtimes that don't support file system access, consider using a remote SQLite service like Turso with its HTTP driver.
