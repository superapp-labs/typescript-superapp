...
When your data source is not a database with a built-in driver -- REST APIs, GraphQL endpoints, edge runtimes, or proprietary systems -- build a custom provider.
...
- **Edge runtimes** -- environments where native drivers cannot be loaded
- **Proprietary databases** -- systems without a built-in driver
- **Data virtualization** -- expose API data as queryable tables alongside SQL databases
...
Pass your custom provider in the `providers` array. Built-in databases (Postgres, MySQL, SQLite) do not need provider imports -- the engine infers them from the URL protocol:
