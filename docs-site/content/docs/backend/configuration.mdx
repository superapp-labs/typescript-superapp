...
The `executor` option controls how queries reach your database. The default is `'duckdb'`, which routes queries through DuckDB. Set `executor: 'direct'` to bypass DuckDB and use native database drivers -- this makes superapp deployable to edge runtimes.

```typescript
// DuckDB mode (default) — cross-database joins, CSV support
const engine = createEngine({
  connections: {
    main: { type: 'postgres', url: process.env.PG_URL! },
  },
...
// Direct mode — edge-compatible, no DuckDB dependency
const engine = createEngine({
  executor: 'direct',
  connections: {
    main: { type: 'postgres', url: process.env.PG_URL! },
...
See [Direct Connections](/docs/advanced/direct-connections) for edge deployment examples and trade-offs.
...
| `executor` | `'duckdb' \| 'direct'` | `'duckdb'` | Query execution mode. `'duckdb'` routes through DuckDB (cross-DB joins, CSV). `'direct'` uses native drivers (edge-compatible). |
...
| `duckdb` | `DuckDBConfig` | — | DuckDB memory, threads, pool settings. Ignored when `executor: 'direct'`. |
...
```typescript
// Development — open access, DuckDB mode
const engine = createEngine({
  connections: {
...
// Development — open access, direct mode (edge-compatible)
const engine = createEngine({
  executor: 'direct',
  connections: {
    main: { type: 'postgres', url: 'postgres://localhost:5432/mydb' },
