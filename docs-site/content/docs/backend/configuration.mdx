...
| `plugins` | `Plugin[]` | `[]` | Plugins that extend the engine. See [Plugins](/docs/backend/plugins) |
...
const engine = createEngine({
  ...
  plugins: [
    stripePlugin({ apiKey: process.env.STRIPE_KEY! }),
    metricsPlugin({ endpoint: 'https://metrics.myapp.com' }),
  ],
  auth: betterAuthProvider({ /* ... */ }),
  permissions: { /* ... */ },
  roles: { /* ... */ },
...
Plugins bundle integrations, auth, permissions, roles, actions, middleware, and routes into a single composable unit. Pass them in the `plugins` array — the engine merges their contributions at startup.

```typescript
import { stripePlugin } from '@superapp/plugin-stripe'
import { metricsPlugin } from '@superapp/plugin-metrics'

const engine = createEngine({
  connections: { main: { type: 'postgres', url: process.env.PG_URL! } },
  plugins: [stripePlugin({ apiKey: '...' }), metricsPlugin({ endpoint: '...' })],
})
```

Plugin contributions merge with your engine config. If both a plugin and your config define the same permission slug or action name, the engine throws at startup with a clear error. Roles from plugins and config are unioned. See [Plugins](/docs/backend/plugins) for the full guide.
