...
```mermaid
flowchart TD
    REQ["Incoming Request — POST /data + Bearer JWT"]
    R1["1. Rate Limiting — Per-user (200/min) and per-IP (500/min)"]
    R2["2. Body Validation — Parse JSON, validate structure"]
    R3["3. JWT Extraction — Decode token from Authorization header"]
    R4["4. Session Resolution — resolveSession(user, db) → enriched user"]
    R5["5. Role Injection — Map user.role → permission slugs"]
    R6["6. Permission Check — Table → Operation → Columns → Filter → Check → Preset"]
    R7["7. Query Builder — JSON request → Kysely → SQL"]
    R8["8. DuckDB Execution — Run SQL against attached databases"]
    R9["9. Audit Log — Record query, params, duration, user"]
    R10["10. Response — Return JSON result to client"]
