...
Any property you return becomes available as a `$user.*` variable in [filters](/docs/backend/permissions/filter), [checks](/docs/backend/permissions/check), and [presets](/docs/backend/permissions/preset).
